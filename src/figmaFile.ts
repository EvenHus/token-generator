...
export async function getFigmaVariables(brand: IBrand): Promise<any> {
    // const response = await fetch(`https://api.figma.com/v1/files/${brand.figmaFileId}/variables/local`, {
    //     method: 'get',
    //     headers: { 'Content-Type': 'application/json', 'X-Figma-Token': `${process.env.FIGMA_PERSONAL_TOKEN}` }
    // });

    // return await response.text();
    return getLocalFile('./public/variables.json');
}
